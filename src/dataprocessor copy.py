...
class AudioDataProcessor():
    ...
    def get_mel_basis(self, fmax=8000):
        ...
        return torch.from_numpy(mel_filterbank).float().to(self.device)  # ts[n_mel, n_freq=n_fft//2+1]
    
    def get_hann_window(self):
        return torch.hann_window(self.win_length).to(self.device)  # ts[win_length,] = [1024,]
    
    def filename_to_wav(self, fname):
        return self.read_audio_file(fname)[3]